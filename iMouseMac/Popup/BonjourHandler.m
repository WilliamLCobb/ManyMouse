...
//static NSString * kWiTapBonjourType = @"_imouse122._tcp.";
...
-(id) init
{
    if ((self = [super init]))
    {
        self.server = [[NSNetService alloc] initWithDomain:@"local." type:kWiTapBonjourType name:@"Mac" port:0];
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)browser didRemoveService:(NSNetService *)service moreComing:(BOOL)moreComing
{
    ...
    if ( ! moreComing ) {
        [self.delegate updatedServices:self.services];
    }
}   
...
- (void)stream:(NSStream *)stream handleEvent:(NSStreamEvent)eventCode
{
    ...
    switch(eventCode) {
        ...
        case NSStreamEventHasBytesAvailable: {
            ...
            bytesRead = [self.inputStream read:&b maxLength:1];
            if (bytesRead <= 0) {
               ...
               [self.delegate recievedData:[[NSData alloc] initWithBytes:&b length:bytesRead]];
