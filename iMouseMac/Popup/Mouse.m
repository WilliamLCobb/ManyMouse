...
-(NSPoint) mousePos
{
    CGPoint mouseLoc = [NSEvent mouseLocation]; //get current mouse position
...
-(void) leftDown
{
    ...
    PostMouseEvent(kCGMouseButtonLeft, kCGEventMouseMoved, [self mousePos]);
    PostMouseEvent(kCGMouseButtonLeft, kCGEventLeftMouseDown, [self mousePos]);
    leftDown = YES;
}
...
{
    PostMouseEvent(kCGMouseButtonLeft, kCGEventMouseMoved, [self mousePos]);
    PostMouseEvent(kCGMouseButtonLeft, kCGEventLeftMouseUp, [self mousePos]);
    leftDown = NO;
}

-(void) rightDown
{
        PostMouseEvent(kCGMouseButtonRight, kCGEventMouseMoved, [self mousePos]);
        PostMouseEvent(kCGMouseButtonRight, kCGEventRightMouseDown, [self mousePos]);
}

-(void) rightUp
{
    //[self shiftMouse:CGPointMake(-1, -1)];
    //PostMouseEvent(0, kCGEventMouseMoved, [self mousePos]);
        PostMouseEvent(kCGMouseButtonRight, kCGEventMouseMoved, [self mousePos]);
        PostMouseEvent(kCGMouseButtonRight, kCGEventRightMouseUp, [self mousePos]);
    
}

-(void) shiftMouse:(CGPoint) point
{
    //point = CGPointMake(pow(point.x, 2) * (point.x/abs(point.x)), pow(point.y, 2) * (point.y/abs(point.y)));
    NSPoint mousePosition = [self mousePos];
    //NSLog(@"%f %f", mousePosition.y, mousePosition.y + point.y);
    NSPoint mouseWarpLocation = NSMakePoint(mousePosition.x + point.x, mousePosition.y + point.y);
    
    if      (mouseWarpLocation.y < 0)
        mouseWarpLocation = CGPointMake(mouseWarpLocation.x, 0);
    else if (mouseWarpLocation.y > [[NSScreen mainScreen] frame].size.height-1)
        mouseWarpLocation = CGPointMake(mouseWarpLocation.x, [[NSScreen mainScreen] frame].size.height-1);
    
    CGWarpMouseCursorPosition(mouseWarpLocation);
    
    if (leftDown)
        PostMouseEvent(kCGMouseButtonLeft, kCGEventLeftMouseDragged, mouseWarpLocation);
    else
        PostMouseEvent(0, kCGEventMouseMoved, mouseWarpLocation);
}

-(void) shiftScrollWheel:(CGPoint) point
{
    CGEventRef cgEvent = CGEventCreateScrollWheelEvent(NULL, kCGScrollEventUnitPixel, 2, point.y, point.x);
...
-(void) sendKey:(NSString *) key
{
    ...
    keydown = CGEventCreateKeyboardEvent (NULL, [self keyCodeFromKeyString:key], true);
    keyup = CGEventCreateKeyboardEvent (NULL, [self keyCodeFromKeyString:key], false);
...
- (CGKeyCode)keyCodeFromKeyString:(NSString *)keyString
{
